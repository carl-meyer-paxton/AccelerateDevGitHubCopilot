...
public static class EnumHelper
{
    ...
    public static string GetDescription(Enum value)
    {
        ...
        return value switch
        {
            LoanExtensionStatus les => LoanExtensionStatusDescriptions.TryGetValue(les, out var desc) ? desc : les.ToString(),
            LoanReturnStatus lrs => LoanReturnStatusDescriptions.TryGetValue(lrs, out var desc) ? desc : lrs.ToString(),
            MembershipRenewalStatus mrs => MembershipRenewalStatusDescriptions.TryGetValue(mrs, out var desc) ? desc : mrs.ToString(),
            _ => value.ToString()
        };
    }
}