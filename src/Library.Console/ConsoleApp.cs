...
public class ConsoleApp
{
    ...
    private async Task<ConsoleState> SearchBooks()
    {
        ...
        var book = await _jsonData.FindBookByTitle(bookTitle!);
        if (book == null)
        {
            Console.WriteLine($"No matching book found for \"{bookTitle}\".");
            return ConsoleState.PatronDetails;
        }

        // Find book items for this book
        var itemIds = _jsonData.BookItems?.Where(bi => bi.BookId == book.Id).Select(bi => bi.Id).ToHashSet() ?? new HashSet<int>();

        // Find any active loan for these items (ReturnDate == null)
        var activeLoan = _jsonData.Loans?.FirstOrDefault(l => itemIds.Contains(l.BookItemId) && l.ReturnDate == null);
